...
### plot the mean cost & CMI trajectory for Figure 1
...
# % change in costs using unadjusted cost 
test2 <- subset(test, variable=="Growth, unadjusted")
test2$int <- ifelse(test2$DischargeDateMonth >= "201405",1,0)
...
ggplot(test2, aes(x=DischargeDateMonth, y=value, group=variable)) +
  ...
  scale_y_continuous(limits = c(-20,20),
                     breaks=seq(-20,20,10)) +
    scale_x_discrete(breaks=c("201109","201201","201205","201209","201301","201305","201309","201401","201405","201409","201501","201505","201509","201601","201605","201609","201701","201705","201709"))
ggsave("~/Dropbox/Research/VBM/results/tvdc_cpi_gr_trajectory.tiff", width=20, height=10, units="cm")
```

additional plots using actual dollar amount as a scale
```{r}
...
fig_2a <- qcc(data =  qcc.groups(test$tvdc_cpi_cmi_gr[test$int==0],
...
fig_2b <- qcc(data =  qcc.groups(test$tvdc_cpi_cmi_gr[test$int==0 & test$pattype=="Medical"],
...
fig_2c <- qcc(data =  qcc.groups(test$tvdc_cpi_cmi_gr[test$int==0 & test$pattype=="Surgical"],
...
# SPC of risk-adjsuted total costs (estimated from running the patient-level analysis) - used for paper as Figure 2
...
fig_2a <- qcc(data =  qcc.groups(racost2$tvdc_cpi[racost2$int==0],
...
fig_2b <- qcc(data =  qcc.groups(racost2$tvdc_cpi_surg0[racost2$int==0],
                                   racost2$DischargeDateMonth[racost2$int==0]),
...
fig_2c <- qcc(data =  qcc.groups(racost2$tvdc_cpi_surg1[racost2$int==0],
                                   racost2$DischargeDateMonth[racost2$int==0]),
...
```{r}
# import unadjusted total costs, which are month fixed effect coefficientss estimated with patient characteristics controls using the discharge-level data in Stata (VBManalysis.do)
unadj_cost <- read.csv("~/Dropbox/Research/VBM/data/unadj_tvdc_cpi_monthly_050918.csv")
library(zoo)
unadj_cost$tt <- as.yearmon(paste(unadj_cost$yr, unadj_cost$month, sep = "-"))
unadj_cost$DischargeDateMonth <- format(unadj_cost$tt, "%Y%m")

colnames(unadj_cost)[which(colnames(unadj_cost)=="unadj_month_all")] <- "tvdc_cpi"
colnames(unadj_cost)[which(colnames(unadj_cost)=="unadj_month_surg0")] <- "tvdc_cpi_surg0"
colnames(unadj_cost)[which(colnames(unadj_cost)=="unadj_month_surg1")] <- "tvdc_cpi_surg1"
unadj_cost <- unadj_cost[c("tvdc_cpi", "tvdc_cpi_surg1", "tvdc_cpi_surg0", "DischargeDateMonth")]
...
# aggregate data
unadj_cost <- merge(unadj_cost, allDates, by="DischargeDateMonth", all.y=TRUE)

# create % change from the pre-intervention mean
ddply(unadj_cost, .(int), summarise, Mall = mean(tvdc_cpi, na.rm=TRUE), Msurg0 = mean(tvdc_cpi_surg0, na.rm=TRUE), Msurg1 = mean(tvdc_cpi_surg1, na.rm=TRUE))
#   int     Mall   Msurg0   Msurg1
# 1   0 2809.575 2626.095 34150.72

unadj_cost$pch <- 100*(unadj_cost$tvdc_cpi - 2809.575)/2809.575
unadj_cost$pch_surg0 <- 100*(unadj_cost$tvdc_cpi_surg0 - 2626.095)/2626.095
unadj_cost$pch_surg1 <- 100*(unadj_cost$tvdc_cpi_surg1 - 34150.72)/34150.72

# time-series data
aggData.ts <- ts(unadj_cost, start=c(2011,9), end=c(2017,12), frequency = 12)
summary(aggData.ts)

plot(aggData.ts[,c("tvdc_cpi")], ylim=c(0,10000) ) # plot avg total variable direct cost
```

SPC for Fig 2 using unadjusted costs
```{r}
library(qcc)
# to implement own version: http://blog.yhat.com/posts/quality-control-in-r.html

unadj_cost2 <- subset(unadj_cost, (DischargeDateMonth < "201209" | DischargeDateMonth > "201308"))

names(unadj_cost2$DischargeDateMonth) <- unadj_cost2$DischargeDateMonth

# normalize the cost by substracting the mean cost and dividing by the SD of the cost during the pre-period (2011/09 - 2012/04) 
monthly_cost <- ddply(unadj_cost2, .(int), summarise, tvdc_cpiM=mean(tvdc_cpi, na.rm=TRUE), tvdc_cpi_surg0M=mean(tvdc_cpi_surg0, na.rm=TRUE), tvdc_cpi_surg1M=mean(tvdc_cpi_surg1, na.rm=TRUE))
# mean cost in pre
# all patients = 9334.434
# med = 5680.629
# surg = 15233.99

# plot % change for all patients
unadj_cost2$tvdc_cpi <-  100*(unadj_cost2$tvdc_cpi - 9334.434)/9334.434
unadj_cost2$tvdc_cpi_surg0 <-  100*(unadj_cost2$tvdc_cpi_surg0 - 5680.629)/5680.629
unadj_cost2$tvdc_cpi_surg1 <-  100*(unadj_cost2$tvdc_cpi_surg1 - 15233.99)/15233.99
...
# all patients
pdf("~/Dropbox/Research/VBM/results/qcc_unadj_cost.pdf")
my.xmr.x <- qcc(data =  qcc.groups(unadj_cost2$tvdc_cpi[unadj_cost2$int==0],
                                   unadj_cost2$DischargeDateMonth[unadj_cost2$int==0]),
                 newdata= qcc.groups(unadj_cost2$tvdc_cpi[unadj_cost2$int==1],
                                     unadj_cost2$DischargeDateMonth[unadj_cost2$int==1]),
                title="(a) All patients",
                type = "xbar.one",
                ylim=c(-15,20),
                ylab="% change (%) in direct variable costs per discharge*",
...
# medical
pdf("~/Dropbox/Research/VBM/results/qcc_unadj_cost_surg0.pdf")
my.xmr.x <- qcc(data =  qcc.groups(unadj_cost2$tvdc_cpi_surg0[unadj_cost2$int==0],
                                   unadj_cost2$DischargeDateMonth[unadj_cost2$int==0]),
                 newdata= qcc.groups(unadj_cost2$tvdc_cpi_surg0[unadj_cost2$int==1],
                                     unadj_cost2$DischargeDateMonth[unadj_cost2$int==1]),
                title="(b) Medical DRGs",
                type = "xbar.one",
                ylim=c(-15,20),
...
# surgical
pdf("~/Dropbox/Research/VBM/results/qcc_unadj_cost_surg1.pdf")
my.xmr.x <- qcc(data =  qcc.groups(unadj_cost2$tvdc_cpi_surg1[unadj_cost2$int==0],
                                   unadj_cost2$DischargeDateMonth[unadj_cost2$int==0]),
                 newdata= qcc.groups(unadj_cost2$tvdc_cpi_surg1[unadj_cost2$int==1],
                                     unadj_cost2$DischargeDateMonth[unadj_cost2$int==1]),
                title="(c) Surgical DRGs",
                type = "xbar.one",
                ylim=c(-15,20),
