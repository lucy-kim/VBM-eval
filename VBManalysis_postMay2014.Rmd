...
# Box.test(aggData.ts[,2], lag = 1, type = "Ljung")
# model <- glm(aggData.ts[,2] ~ time + intervention +  timeAfterIntervention, data = aggData.ts[,2])
# durbin.watson(model)
...
my.xmr.x <- qcc(data =  qcc.groups(racost2$tvdc_cpi[racost2$int==0],
                ...
                type = "xbar.one",
                ylim=c(-15,15),
                ylab="Percentage change (%) from pre-intervention mean",
                xlab="Month",
                axes.las = 1,
                # add.stats = F,
                label.limits = c("","")) 
...
v <- c("Hospital", "Discharge Quarter","Mean Total Cost (Obs)","Mean Direct Cost (Obs)","CMI", "Cases.x", "Cases.y", "Direct Cost Cases Reported")
df <- df[v]
...
actual percentile score in each quarter based on mean direct cost divided by CMI
```{r}
vizdf$mdcost_adj <- vizdf$mdcost/vizdf$CMI

library(dplyr)

test <-  vizdf %>%
 group_by(qt) %>% 
 mutate(percrank=rank(mdcost_adj)/length(mdcost_adj))

test <- test[,c("qt","Hospital","percrank","mdcost_adj","nyu")]
subset(test, nyu==1)
```
