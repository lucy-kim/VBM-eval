...
# load("~/Box Sync/VBM/Data/vbmR/sample-full2017-postMay2014 (Lucy.Kim@nyumc.org 2).RData")
...
Exclude LOS = 1 day and reporting department = Medicine - SKIP: Don't apply this rule any more
```{r}
...
# run SAS codes to create elixhauser comorbidities
...
```{r}
v <- c("patid","DischargeDateMonth","tvdc_cpi", "dx1","dx2", "dx3", "dx4", "dx5", "dx6", "dx7", "dx8", "dx9", "dx10", "dx11", "dx12", "dx13", "dx14", "dx15", "dx16", "dx17", "dx18", "dx19", "dx20", "dx21", "dx22", "dx23", "dx24", "dx25", "dx26", "dx27", "dx28", "dx29", "cmiD", "surgical", "female", "AgeGroup", "raceGroup", "ins", "t", "int", "tpostInt", "season")
test <- patUse3[v]
...
for (i in 1:29) {
  x <- paste0("dx",i)
  patUse3$dxsum <- patUse3$dxsum + patUse3[,x]   
}
summary(patUse3$dxsum)

keep <- c("CMI", "dxsum", "tvdc_cpi", "DischargeDateMonth", "int")
test3 <- ddply(patUse3[keep], .(DischargeDateMonth), numcolwise(mean, na.rm=TRUE))

for (col in c("CMI", "dxsum", "tvdc_cpi")) {
  test3[test3$DischargeDateMonth >= "201209" & test3$DischargeDateMonth <= "201308", col] <- NA
}

ggplot(test3, aes(x=DischargeDateMonth, y=dxsum, group=1)) +
  geom_line() +
  geom_point() +
  theme_bw(base_size=13) +
  ...
  ylim(0,0.5) +
  scale_x_discrete(breaks=c("201109","201201","201205","201209","201301","201305","201309","201401","201405","201409","201501","201505","201509","201601","201605","201609","201701","201705","201709"))
ggsave("~/Dropbox/Research/VBM/results/dxsum_trajectory.pdf", width=20, height=10, units="cm")

# by patient type
keep <- c("CMI", "dxsum", "tvdc_cpi", "DischargeDateMonth", "pattype")
...
ggplot(test3, aes(x=DischargeDateMonth, y=dxsum, group=pattype, colour=pattype)) +
  geom_line() +
  geom_point() +
  theme_bw(base_size=13) +
  labs(x="Month", y="Elixhauser comorbidity index",
...
ggsave("~/Dropbox/Research/VBM/results/dxsum_trajectory_bypattype.pdf", width=20, height=10, units="cm")
...
pdf("~/Dropbox/Research/VBM/results/qcc_tvdc_cpi_cmi_gr.tiff")
...
# surgical
pdf("~/Dropbox/Research/VBM/results/qcc_tvdc_cpi_cmi_gr_surg.tiff")
...
# medical
pdf("~/Dropbox/Research/VBM/results/qcc_tvdc_cpi_cmi_gr_med.tiff")
